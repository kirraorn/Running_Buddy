...
return None
return r.json()
def paginate(url: str, token: str, params: Optional[Dict[str, Any]] = None) ->
List[Any]:
items: List[Any] = []
page = 1
while True:
...
return owner, name
def ensure_labels(owner: str, repo: str, token: str, labels: List[Dict[str, Any]])
-> None:
...
cur = by_name[name]
if cur.get("color", "").lower() != color.lower() or
(cur.get("description") or "") != desc:
...
def ensure_milestones(owner: str, repo: str, token: str, milestones: List[Dict[str,
Any]]) -> Dict[str, int]:
# include closed milestones too, so reruns still find them
existing = paginate(f"{GITHUB_API}/repos/{owner}/{repo}/milestones", token,
params={"state": "all"})
...
def find_issue_by_backlog_id(owner: str, repo: str, token: str, backlog_id: str) ->
Optional[int]:
...
data = request_json("GET", f"{GITHUB_API}/search/issues", token, params={"q":
q, "per_page": 5})
...
die(f"Issue {backlog_id} references unknown milestone:
{milestone_title}")
...
if not token:
die("GITHUB_TOKEN not set (did you run in GitHub Actions with
permissions?)")
...
print(f"Labels: {len(labels)}, milestones: {len(milestones)}, issues:
{len(issues)}")
if labels:
